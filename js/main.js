...
function renderProjects() {
  ...
  grid.innerHTML = PROJECTS.map((p, i) => {
    ...
    const chips = p.stack.map(s =>
      `<span class="font-mono text-[11px] px-2.5 py-1 rounded-md bg-white/5 border border-white/10 text-gray-400">${s}</span>`
    ).join('');
    const confBadge = p.confidential
      ? `<span class="font-mono text-[11px] px-2.5 py-1 rounded-full bg-white text-black font-medium">confidentiel</span>`
      : '';
    const ghLink = p.github
      ? `<a href="${p.github}" target="_blank" rel="noopener noreferrer" aria-label="Voir sur GitHub" class="text-gray-400 hover:text-white transition-colors" onclick="event.stopPropagation()">${githubIcon()}</a>`
      : '';
    const imgCount = p.images.length > 1
      ...
      : '';
    const cover = p.cover ? `
      <div class="relative h-52 ${p.size === 'lg' ? 'md:h-64' : ''} overflow-hidden rounded-t-2xl bg-[#11111c]">
        <img src="${p.cover}" alt="${p.title}" loading="lazy"
             class="card-img w-full h-full ${p.coverFit === 'contain' ? 'object-contain p-6' : 'object-cover object-top'}">
        <div class="absolute inset-0 bg-gradient-to-t from-[#0a0a12] via-transparent to-transparent opacity-80"></div>
        ${imgCount}
      </div>` : '';

    return `
    <article class="reveal tilt-card glass rounded-2xl overflow-hidden ${wide} ${p.images.length ? 'cursor-pointer' : ''}"
             ${p.images.length ? `onclick="openLightbox(PROJECTS[${i}].images, 0)" role="button" tabindex="0" aria-label="Voir la galerie de ${p.title}"` : ''}>
      ${cover}
      <div class="p-7">
        <div class="flex items-start justify-between gap-4 mb-3">
          <span class="font-mono text-xs px-3 py-1 rounded-full bg-violet-500/10 border border-violet-500/30 text-violet-300">${p.badge}</span>
          <div class="flex items-center gap-3">${confBadge}${ghLink}</div>
        </div>
        <h3 class="font-display text-xl md:text-2xl font-semibold mb-2 text-white">${p.title}</h3>
  ...
  // Enter/Espace ouvre la galerie au clavier
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Compteurs animés (stats Jeunes Capucins)
